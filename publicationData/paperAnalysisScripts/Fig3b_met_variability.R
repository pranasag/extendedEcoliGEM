...
glc_normalized <- subset(glc_normalized, (glc_normalized$cluster %in%
                                            c('Met Metabolism','Thr and Lys Metabolism', 'Val, Leu, and Ile Metabolism')))
...
my_breaks <- function(x) { if (max(x) < 3) seq(0.0, 1.0, 0.2) else (seq(0, 20, 5))}
my_limits <- function(x) { if (max(x) < 3) c(-0.05,1) else (c(-1, 20))}
...
g <- ggplot(glc_normalized, aes(ymin = min, ymax = max, y=min, x = condition, col = condition, fill = Proteomics)) + 
  ...
  scale_y_continuous(breaks = my_breaks, limits = my_limits, expand  = c(0.0,0.0)) +
